...
def graphs(memory, cycles):
    for cyc, mem in zip(cycles, memory):
        plt.scatter([cyc] * len(mem), mem)
...
def main():
    ...
    while True:
        ...
        if function == 'add':
            ...
            mem_index = ld(instn[pc])
            memory_address.append((pc, mem_index))
            print(format(pc))
            pc += 1

        elif function == 'st':
            mem_index = st(instn[pc])
            memory_address.append((pc, mem_index))
            print(format(pc))
            pc += 1
