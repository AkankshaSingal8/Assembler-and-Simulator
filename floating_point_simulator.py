...
def bin_to_dec(bin, leng) :
    ...
    for i in range(dot-1, -1, -1) :
        intDec += ((ord(bin[i]) -ord('0')) * tw)
        tw *= 2
 
    tw = 2
    for i in range(dot + 1, leng):
        fracDec += ((ord(bin[i]) - ord('0')) / tw);
        tw *= 2.0
    an = intDec + fracDec
    return an
...
def binfloat_to_regformat(binum:str)->str:
    ...
    if len(binum)<= 7:
        ...
        if exponent >7 or exponent<0:
            return 'invalid'
        bin_exponent = '0'*(3-len(str(int(dec_to_bin(exponent))))) + str(int(dec_to_bin(exponent)))
        lst.pop(0)
...
def binfloatreg_to_dec(num:str)->float:
    mantissa = num[3:]
    exp = bin_to_dec(num[:3],3)
    right_shift = exp
    no = float('1.'+ mantissa)
    bin_num = no * 10**exp
    return bin_to_dec(str(bin_num), len(str(bin_num)))
...
def addf(ins:str):
    ...
    sc_reg1_DecVal = binfloatreg_to_dec (reg_val[sc_reg1_name])
    sc_reg2_DecVal = binfloatreg_to_dec (reg_val[sc_reg2_name])
    
    sum_r1_r2 = sc_reg1_DecVal + sc_reg2_DecVal

    if sum_r1_r2 > 252 or sum_r1_r2 < 1:
        list_val = list(reg_val['FLAGS'])
        list_val[-4]='1'
        new_flag= ''.join(list_val)
        reg_val['FLAGS'] = new_flag
        reg_val[dest_reg_name] = (binfloat_to_regformat(float_to_bin(max(sc_reg1_DecVal,sc_reg2_DecVal))))

    else:
        reg_val[dest_reg_name] = binfloat_to_regformat(float_to_bin((sum_r1_r2)))
...
def subf(ins:str):
    ...
    diff_r1_r2 = sc_reg1_DecVal - sc_reg2_DecVal
    ...
    if  diff_r1_r2 < 1:
        ...
        new_flag= ''.join(list_val)
        reg_val['FLAGS'] = new_flag
        reg_val[dest_reg_name] = (binfloat_to_regformat(float_to_bin(min(sc_reg1_DecVal,sc_reg2_DecVal))))

    else:
        reg_val[dest_reg_name] = binfloat_to_regformat(float_to_bin((diff_r1_r2)))
...
def movf(ins:str):
    ...
    reg_name = bin_to_reg(ins[5:8])
    float_val = ins[-8:]
    reg_val[reg_name] = '0'*8 + float_val
